...
## 🌳 Estimadores da AAS
### 🤔 População Infinita x População Finita
<br>

.pull-left-4[
.font80[
- **População Infinita (na prática)**

**Definição**: A amostra ($n$) representa uma .blue[pequena fração] do universo total de .blue[unidades amostrais possíveis na população] ($N$).
<br><br>

**Exemplo**: Uma floresta possui 100.000 parcelas possíveis ($N$ = 100.000). Se forem medidas 20 parcelas ($n$ = 20), a amostra representa apenas uma pequena fração da população.
]
]
...
**Definição**: A amostra ($n$) representa uma .blue[parcela relevante da população], cobrindo uma .blue[fração significativa] do universo total de unidades amostrais possíveis na população ($N$).
<br><br>

**Exemplo**: Uma floresta possui 100 parcelas possíveis ($N$ = 100). Se forem medidas 20 parcelas ($n$ = 20), a amostra representa uma parcela relevante da população.
]
]

.pull-right-15[
<br>
.center[.font90[**Portanto, a definição de população finita ou infinita depende da .blue[fração amostral].**]]
]
...
- A decisão é baseada na fração amostral:
<br>

**Critério prático**:
- Se $f$ $≤$ 2% → população tratada como .blue[infinita]
- Se $f$ $>$ 2% → população tratada como .blue[finita]
]
]
...
.shadow1[
🤔 **O Papel do Fator de Correção (FC)**?
.font80[
- **Redução da incerteza em populações finitas**: Em populações finitas, a incerteza diminui à medida que mais unidades amostrais são medidas. O conhecimento sobre a população florestal aumenta proporcionalmente ao esforço amostral de campo.
- **Evitar a superestimativa do erro**: O FC ajusta o erro padrão da média (EPM), evitando superestimar a incerteza quando a amostra representa uma parcela relevante da população.
- **Conclusão prática**: Quanto maior a fração amostral, menor será a incerteza sobre a população restante (não amostrada). Nesse cenário, maior será a importância do fator de correção (FC).
]
]
