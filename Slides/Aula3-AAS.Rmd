...
.pull-left-9[
**🤔 Mas, qual é a idéia básica do IC?**
.font90[
- Suponha o interesse em estimar o parâmetro populacional desconhecido $\mu$ de uma variável de interesse (Ex.: volume).
- Então, se 100 amostras aleatórias (de mesmo tamanho n) da população fossem realizadas, se espera que 95% de todos os ICs construídos conteriam o parâmetro populacional desconhecido $\mu$.
]
]
...
- Suponha que a variável de interesse é o .blue[volume de madeira por hectare] em uma população florestal de tamanho conhecido.
- **Você teria duas opções**: censo florestal ou inventário por amostragem.
- **Perfeito!** Então, suponha a decisão por realizar um inventário por amostragem. Portanto, uma .blue[estimativa do valor do parâmetro] $\color{blue}{\mu}$ será obtida usando dados de parcelas amostrais.
- **Resultado**: ter-se-á uma estimativa do parâmetro populacional $\mu$ (Ex.: $\bar{X} = 144~m^3.ha^{-1}$). Nesse caso, o valor de $\bar{X} = 144~m^3.ha^{-1}$ representa o valor mais plausível do parâmetro $\mu$, e depende da amostra. Portanto, denomina-se esse valor de uma .blue[estimativa pontual de] $\color{blue}{\mu}$. 
- **Fato**: Essa estimativa depende da amostra
]
]
