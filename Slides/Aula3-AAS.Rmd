...
- Suponha uma população florestal de *Swietenia macrophylla*, implantada sob o espaçamento 5m x 5m (Área Total = 10ha).
...
- Retornemos ao exemplo da população florestal de *Swietenia macrophylla*...
...
.pull-left-4[
.font90[
- Suponha uma população florestal de *Swietenia macrophylla* com área total de 100ha e a implantação de 5 parcelas de 100m x 100m. Pergunta-se:
]
]
...
.font90[
**Estimador**: é o desvio padrão amostral dividido pela raiz quadrada do número de observações.
...
.font90[
- Vamos supor a seleção aleatória de duas UAs (2 e 7).
]
]
...
.pull-left-9[
.font90[
- Porém, veja que a amostra sorteada é apenas uma possibilidade dentre várias combinações possíveis de amostras de mesmo tamanho.
]
]
...
.pull-left-9[
.font90[
- Portanto, a combinação simples de n = 10 elementos de tamanhos k = 2 é dada por:<br>
...
.font70[
**Interpretação**: Seria possível obter 45 amostras diferentes de tamanho k = 2 a partir de 10 UAs.
]
...
.pull-left-4[
.font90[
.center[**Combinações possíveis?**]
- Na linguagem R, use as funções:

.green[choose()] = para obter o número de combinações possíveis.

.green[combn()] = para visualizar as combinações.
]

```{r comb, echo=T, eval=F}
choose(10, 2)
t(combn(1:10, 2))
```
]
...
.pull-right-4[
.scroll-box-20[
```{r ref.label="comb", echo=F, eval=T, collapse=T}
```
]
]
